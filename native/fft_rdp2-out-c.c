...
#define DESCRIPTION_STR "Real double (COMPLEX_REAL + PACK_FORMAT), not in-place, cached"
...
#pragma omp parallel for simd
            for (k_comp = N, k_dest = 0; k_dest < N; k_dest++, k_comp--) {
                ...
                if (k_dest == 0) {
                    buf[k_dest].real = y[0];
                    buf[k_dest].imag = 0.0;
                } else if (k_dest == k_comp) {
                    buf[k_dest].real = y[2 * k_dest - 1];
                    buf[k_dest].imag = 0.0;
                } else if (k_dest < k_comp) {
                    buf[k_dest].real = y[2 * k_dest - 1];
                    buf[k_dest].imag = y[2 * k_dest];
                } else {
                    buf[k_dest].real = y[2 * k_comp - 1];
                    buf[k_dest].imag = -y[2 * k_comp];
                }
            }
