...
perf_times = time_func(np.fft.fft, mat_z, dict(axis=0), refresh_buffer=False)
print_summary(perf_times, header='np.fft.fft(axis=0)' + arg_signature(mat_z))

tmp = arr_z.T
perf_times = time_func(np.fft.fft2, tmp, dict(), batch_size=8, refresh_buffer=False)
print_summary(perf_times, header='np.fft.fft2' + arg_signature(tmp))

tmp = arr_z.transpose((2,0,1))
perf_times = time_func(np.fft.fftn, tmp, dict(), batch_size=8, refresh_buffer=False)
